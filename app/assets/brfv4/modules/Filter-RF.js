...
qcBrfv4.loadModules = function() {
	...
	//qcNetwork.localJSON("./assets/brfv4/assets/json/SF_Giants.json").then((data) => {
	qcNetwork.localJSON(self.localJsonFile).then((data) => {
		...
		for(let idx = 0; idx < self._texturesData.length; idx++) {
			qcBrfv4._texturesData[idx]._faceTex = qcBrfv4._texturesData[idx].moduleData.marcel_0;
		}
		qcLog.log(`Brfv4 Loaded Bitmap`, 4);
		return self.loadExtendJS();
	}).then(() => {
...
qcBrfv4.changeModules = function(idx) {
	...
	if(typeof self._texturesData[idx] !== 'undefined') {
		self._faceTex = self._texturesData[idx]._faceTex;
		self._texture.src = self._texturesData[idx]._faceTex.tex;
