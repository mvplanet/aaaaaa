	...
	if(qcBrfv4._faceTexturesReady) {
		...
		for (let i = 0; i < faces.length; i++) {
			...
			if (face.state === qcBrfv4.brfv4.BRFState.FACE_TRACKING_START ||
				...
				qcBrfv4.drawFaceClip(face.vertices);

				let triangles = face.triangles.concat();
...
qcBrfv4.drawFaceClip = function(vertices) {
	...
	ctx.lineTo(vertices[26*2], vertices[26*2+1]); 
	ctx.lineTo(vertices[25*2], vertices[25*2+1]); 
	ctx.lineTo(vertices[24*2], vertices[24*2+1]); 
	ctx.lineTo(vertices[23*2], vertices[23*2+1]); 
	//ctx.lineTo(vertices[22*2], vertices[22*2+1]); 
	//ctx.lineTo(vertices[21*2], vertices[21*2+1]); 
	ctx.lineTo(vertices[20*2], vertices[20*2+1]); 
	ctx.lineTo(vertices[19*2], vertices[19*2+1]); 
	ctx.lineTo(vertices[18*2], vertices[18*2+1]); 
	ctx.lineTo(vertices[17*2], vertices[17*2+1]); 
